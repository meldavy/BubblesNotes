...
class URLPreviewServiceIntegrationTest {
    ...
    @Test
    fun `cache hit should return cached preview without network call`() = runTest {
        var requestCount = 0
    ...
    @Test
    fun `cache miss should fetch from network`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
            respond(
    ...
    @Test
    fun `cache expiration should trigger network fetch`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
            respond(
        ...
        class TestURLPreviewService(client: HttpClient, duration: Long) : URLPreviewService(client, duration) {
            ...
            fun advanceTime(ms: Long) { manualTime += ms }
        }
    ...
    @Test
    fun `clearCache should remove specific URL from cache`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
            respond(
        ...
        // First fetch
        service!!.fetchPreview("https://example.com/clear-test")
        assertEquals(1, requestCount)
        ...
        service!!.clearCache("https://example.com/clear-test")
    ...
    @Test
    fun `clearCache should remove entire cache`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
            respond(
                content = "<html><head><meta property=\"og:title\" content=\"Title $requestCount\"></head></html>",
                status = HttpStatusCode.OK,
                headers = headersOf(HttpHeaders.ContentType, "text/html")
            )
        }
        ...
        service!!.fetchPreview("https://example.com/page1")
        assertEquals(2, requestCount) // No new request
    ...
    @Test
    fun `timeout should return empty preview after 5 seconds`() = runTest {
    ...
    @Test
    fun `normal response should complete within timeout`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
                content = """
                    <html>
    ...
    @Test
    fun `fetchPreview should extract Open Graph metadata`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
                content = """
                    <html>
    ...
    @Test
    fun `fetchPreview should fallback to regular meta tags when OG tags missing`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
                content = """
                    <html>
    ...
    @Test
    fun `fetchPreview should handle missing metadata gracefully`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
    ...
    @Test
    fun `fetchPreviewsFromContent should extract and fetch all URLs from content`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
        ...
        service = URLPreviewService(HttpClient(mockEngine), cacheDurationMs = 60000L)
        val content = "Check [Example](https://example.com) and https://test.org/page"

        val previews = service!!.fetchPreviewsFromContent(content)

        assertEquals(2, previews.size)
        assertEquals(2, requestCount)
        assertTrue(previews.any { it.title?.contains("example.com") == true })
        assertTrue(previews.any { it.title?.contains("test.org") == true })
    }

    // ==================== Error Handling Tests ====================

    @Test
    fun `fetchPreview should handle HTTP error responses`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
                content = "Not Found",
    ...
    @Test
    fun `fetchPreview should handle server errors gracefully`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
    ...
    @Test
    fun `fetchPreview should handle empty response body`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
                content = "",
                status = HttpStatusCode.OK,
    ...
    @Test
    fun `fetchPreview should handle malformed HTML gracefully`() = runTest {
        val mockEngine = MockEngine { request ->
            respond(
    ...
    @Test
    fun `normalized URLs should use same cache entry`() = runTest {
        var requestCount = 0
        val mockEngine = MockEngine { request ->
            requestCount++
            respond(
    ...
    @Test
    fun `truncateIfNeeded should truncate long titles`() = runTest {
        val mockEngine = MockEngine { request ->
    ...
    @Test
    fun `truncateIfNeeded should truncate long descriptions`() = runTest {
        val mockEngine = MockEngine { request ->
