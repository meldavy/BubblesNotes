...
class NotesApiIntegrationTest {
    ...
    @Test
    fun `test create note`() = testApplication {
    ...
    @Test
    fun `test get all notes with pagination`() = testApplication {
        ...
        val response = client.get("/api/v1/notes?limit=3") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
    ...
    @Test
    fun `test infinite scroll pagination`() = testApplication {
        val now = System.currentTimeMillis()
        ...
        val firstResponse = client.get("/api/v1/notes?limit=5") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        ...
        val secondResponse = client.get("/api/v1/notes?limit=5&cursor=$cursor") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
    ...
    @Test
    fun `test get note by id`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
            title = "Single Note",
        ...
        val response = client.get("/api/v1/notes/$noteId") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    @Test
    fun `test get non-existent note returns 404`() = testApplication {
        val mockUserId = testUserId.toString()
        ...
        val response = client.get("/api/v1/notes/999999") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
    ...
    @Test
    fun `test update note`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
        ...
        val response = client.put("/api/v1/notes/$noteId") {
            contentType(ContentType.Application.Json)
            cookie("session-auth", getSessionCookie(mockUserId))
            setBody(
                NoteUpdateRequest(
    ...
    @Test
    fun `test update note with partial data`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
            title = "Original Title",
    ...
    @Test
    fun `test update non-existent note returns 404`() = testApplication {
        val mockUserId = testUserId.toString()
        ...
        val response = client.put("/api/v1/notes/999999") {
            contentType(ContentType.Application.Json)
            cookie("session-auth", getSessionCookie(mockUserId))
            setBody(
                NoteUpdateRequest(
    ...
    @Test
    fun `test delete note`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
        ...
        val getResponse = client.get("/api/v1/notes/$noteId") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.OK, getResponse.status)

        val deleteResponse = client.delete("/api/v1/notes/$noteId") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.NoContent, deleteResponse.status)

        val verifyResponse = client.get("/api/v1/notes/$noteId") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
    ...
    @Test
    fun `test delete non-existent note returns 404`() = testApplication {
        val mockUserId = testUserId.toString()
        ...
        val response = client.delete("/api/v1/notes/999999") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.NotFound, response.status)
    }
    ...
    @Test
    fun `test get URL previews for note with previews`() = testApplication {
        val previewData = jsonSerializer.encodeToString(
            listOf(
                URLPreview(
        ...
        val response = client.get("/api/v1/notes/$noteId/previews") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    @Test
    fun `test get URL previews for note without previews`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
    ...
    @Test
    fun `test get URL previews for non-existent note returns 404`() = testApplication {
        val mockUserId = testUserId.toString()
        ...
        val response = client.get("/api/v1/notes/999999/previews") {
            cookie("session-auth", getSessionCookie(mockUserId))
        }
        assertEquals(HttpStatusCode.NotFound, response.status)
    }
    ...
    @Test
    fun `test get URL previews for note with invalid preview data`() = testApplication {
        val note = Note(
            id = 0,
            userId = testUserId,
    ...
    @Test
    fun `test create note with markdown links extracts URLs`() = testApplication {
        val mockUserId = testUserId.toString()
    ...
    @Test
    fun `test create note with empty title uses null`() = testApplication {
        val mockUserId = testUserId.toString()
    ...
    @Test
    fun `test create note with required content only`() = testApplication {
        val mockUserId = testUserId.toString()
